# DEPRECATED: 已用 agent_langgraph.py (LangGraph create_react_agent) 替代
# 保留此文件仅作参考，不再被 app.py 调用
...
from agent_langgraph import agent_executor as langgraph_executor
import os
...
def agent_executor(query_text: str, agent=False):
    ...
    if agent:
        ...
        # )
        tools.extend([date_tool, calculator_tool])
    llm = ChatOpenAI(
